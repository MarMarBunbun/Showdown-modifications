...
function runDexsearch(target, cmd2, canAll, message, isTest) {
  ...
  for (const species of mod.species.all()) {
    ...
    if (species.gen <= mod.gen && (nationalSearch && species.isNonstandard && !["Custom", "Glitch", "Pokestar", "Future"].includes(species.isNonstandard) || (species.tier !== "Unreleased" || unreleasedSearch) && species.tier !== "Illegal") && (!species.tier.startsWith("CAP") || capSearch) && megaSearchResult && gmaxSearchResult && fullyEvolvedSearchResult) {
